...
def get_pdf_path(subject, level, paper=None):
    ...
    if subject == "agriculture":
        ...
        if level == "higher":
            ...
            return paper1, paper2    # Add more mappings for other subjects/levels
    return None
...
async def generate_questions(data: TopicRequest):
    ...
    if data.subject == "business":
        ...
        if not result or len(result) != 2:
            raise HTTPException(status_code=404, detail="Past exam paper not found. business 1")
        paper1_pdf_path, paper2_pdf_path = result
        if not os.path.exists(paper1_pdf_path) or not os.path.exists(paper2_pdf_path):
            raise HTTPException(status_code=404, detail="Past exam paper not found. business 2")
        ...
        if not os.path.exists(pdf_path):
            raise HTTPException(
                status_code=404, 
                detail=f"Past exam paper not found at path: {pdf_path}"
            )
